...
class XTime
{
    ...
    public:
        ...
        // Ex: 0.5 would mean that the second sample would be 50% less important the the first and the third would  be 25% and
        // so on.
        ...
        double Delta();
        // Because the delta is one frame behind and not a linear change, visual motion can appear rough compared to fixed time
        // steps. This method can be used to reterive a more granular version of the delta.(Better for motion) This is acheived
        // by using previous Delta values to compute a weighted running average.
        double SmoothDelta();
